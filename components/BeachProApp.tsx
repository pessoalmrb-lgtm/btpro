...
export default function BeachProApp() {
  ...
  // null = ainda não consultado nesta sessão; o snapshot do Firestore
  // só comanda isPremium enquanto este ref for null.
  const rcPremiumRef = useRef<boolean | null>(null);
  ...
  // --- Assinatura: valida com o RevenueCat no login/startup (app nativo) ---
  React.useEffect(() => {
    if (!user || user.isAnonymous) {
      rcPremiumRef.current = null;
      return;
    }
    let cancelled = false;
    (async () => {
      const status = await fetchSubscriptionStatus(user.uid);
      if (cancelled || status === null) return; // web ou falha de rede: mantém Firestore
      rcPremiumRef.current = status.isPremium;
      setIsPremium(status.isPremium);
      mirrorExpirationToFirestore(user.uid, status);
    })();
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userDocRef, async (snapshot) => {
      if (snapshot.exists()) {
        ...
        // No app nativo, o RevenueCat é a fonte da verdade do premium.
        // O Firestore só comanda enquanto o RC não respondeu nesta sessão.
        setIsPremium(rcPremiumRef.current ?? (userData.isPremium || false));
            ...
            const variants = [
              ...
              () => (
                ...
                >
                  ...
                  >
                    <div className="bg-slate-900 px-7 pt-8 pb-6 text-center relative">
                      <button onClick={close} className="absolute top-4 right-4 p-2 text-white/30 hover:text-white/60 transition-colors">
                        <X size={18} />
                      </button>
              ...
              () => (
                ...
                >
                  ...
                  >
                    <div className="px-7 pt-8 pb-4 text-center relative">
                      <button onClick={close} className="absolute top-4 right-4 p-2 text-white/30 hover:text-white/60 transition-colors">
                        <X size={18} />
                      </button>
